...
\title{${2.267061 \text{grams} \over \text{1.39}\times 10^{24}} \approx \sqrt{k_e^2~q_e^4~T \over c~G~M~L^2}= \sqrt{c~ \hbar^2 ~\alpha^2 ~T\over G~M~L^2}=M_F$ }
...
\noindent\section {Dimensionality}Stoney-Mass $M_{{\text{S}}} = \sqrt{\hbar ~ \alpha ~c \over G}$ units are correct, but with $M_F = M_S \sqrt{\hbar \alpha}$ alone gives correct arithmetic;
 for all the equations below to not only work arithmeitc wise, $\sqrt{T \over M^{1}L^{2}}$ is needed cancel any wrong units put into $M_F$ from $\hbar$. M, L, and T with no subscripts are meant to mean from whichever unit system you are converting from, in this case; SI units 1kg, 1m, and 1s respectively.
...
\end{align*}
Perhaps one day for the sake of simplicity, Bureau international des poids et mesures might redefine the second and meter such the $D_C$ is exactly $1.39\times 10^{24}$ rather than approx. 5 almost 6 nines.
\section{Conclusion}
...
{\hbar \over c^2 M_F} = T_F \approx ~{1 \text{Second}\over \text{1.39}\times 10^{24}};\quad Q_F &\approx~{222702.257\text{Coulumbs}\over \text{1.39}\times 10^{24}}\\
{\hbar \over c M_F} = L_F  &\approx ~{299792458 \text{Meters}\over \text{1.39}\times 10^{24}}
\end{align*}
