...
%\title{${2.267061 \text{grams} \over \text{1.39}\times 10^{24}} \approx {{c ~ T_{SI}\over L_F}M_F \over \text{1.39}\times 10^{24}} \approx \sqrt{k_e^2~q_e^4~T \over c~G~M~L^2}= \sqrt{c ~\alpha^2 ~ \hbar^2 ~T\over G~M~L^2}=M_F$ }
\title{The Pfund Mass $M_F=\sqrt{k_e^2~q_e^4~T \over c~G~M~L^2}= \sqrt{c ~\alpha^2 ~ \hbar^2 ~T\over G~M~L^2}$}
%\notitle{}
\author{\copyright Roy Pfund\thanks{e-mail: r0ypfund@gm41l.c0m} ~(CC BY-NC-ND 4.0) 2022\\
\url{https://creativecommons.org/licenses/by-nc-nd/4.0/}}
%\date{} % Activate to display a given date or no date (if empty), otherwise the current date is printed
\date{$M_F$ equation finished March 17, 2022 Paper updated January 16, 2026}
\begin{document}
...
%$$\sqrt{k_e^2~q_e^4~T \over c~G~M~L^2}= \sqrt{c ~\alpha^2 ~ \hbar^2 ~T\over G~M~L^2}=M_F$$
%$$\sqrt{k_e^2~q_e^4~T \over c~G~M~L^2}= \sqrt{c ~\alpha^2 ~ \hbar^2 ~T\over G~M~L^2}=M_F$$
%$${2.267061 \text{grams} \over \text{1.39}\times 10^{24}} \approx {{c ~ T_{SI}\over L_F}M_F \over \text{1.39}\times 10^{24}} \approx \sqrt{k_e^2~q_e^4~T \over c~G~M~L^2}= \sqrt{c ~\alpha^2 ~ \hbar^2 ~T\over G~M~L^2}=M_F$$
Here we treat the Dalton not as a value to be "found" through physical 
artifacts, but as an algebraic identity emerging from constants of the 
universe. We transition from measuring string to using math to solve 
for the constant. It is possible to further reduce the number of constants in use ~$M_F^{W}L_F^{X}T_F^{Y}Q_F^{Z}=$% (besides masses of subatomic particles).
...
c = ~1 & ~L_F^{1}T_F^{-1}\\
\textcolor{red}{N_A = ~1 }&\textcolor{red}{ ~M_F^{1}\text{**}}\\
...
The SI definition of ${M~8~V \over m~a^3} = N_A$ (Avogadro constant) and $\frac{2 R_\infty h}{c \alpha^2 A_r(e)}=m_u$ (the Dalton), 
remains experimental, relying on physical measurements to approximate 
a fundamental ratio rather than deriving it as an algebraic identity. 
Scientists must count atoms by measuring the sphere's mass, volume, 
and lattice parameter. Determining $N_A$ via silicon sphere measurement resembles measuring 
string wrapped around a circle to calculate $\pi$.
%$N_A = {M~8~V \over m~a^3}\quad; \qquad m_u = \frac{2 R_\infty h}{c \alpha^2 A_r(e)}$
...
% and reduce the constants people have to know.
...
\noindent\section {Dimensionality}Stoney-Mass $M_{{\text{S}}} = \sqrt{\hbar ~ \alpha ~c \over G}$ units are correct, but with $M_F = M_S \sqrt{\hbar \alpha}$ alone gives correct arithmetic;
 for all the equations above to not only work arithmetic wise, $\sqrt{T \over M^{1}L^{2}}$ is needed to cancel any wrong units put into $M_F$ from $\hbar$. M, L, and T with no subscripts are meant to mean from whichever unit system you are converting from or currently using, in this case; SI units 1kg, 1m, and 1s respectively.
%$$\hbar~\alpha^2 = {10^{-14} Q^4_F\over M_F T_F} ={G \over ~M_F^{3}L_F^{-1}T_F^{-2}Q_F^{0}}$$
$\sqrt{T \over M^{1}L^{2}}$ reappears in the solution for $G$. However units for $G$ are wrong because it's not a force, it is the curvature of space time.
$$\alpha^2\hbar= \alpha^2 {M_F^{}L_F^{2} \over T_F} ={G \over ~M_F^{3}L_F^{-1}T_F^{-2}}$$

\section{Time is On Our Side(\& Distance)}
$${c ~ T_{SI}\({G \over c^2 }M_F{M~L^2 \over \alpha^2 \hbar ~T }\)}^{-1}=D_C\qquad\quad{2.267061 \text{grams} \over D_C } \approx M_F$$
%$${1.3899982\times 10^{24}  \approx {c ~ T_{SI}\over L_F} =D_C \over1.39\times 10^{24}}=0.999998675$$
With a Sympathetic Constant $=D_C={c ~ T_{SI}\over L_F}=$ light-second / $L_F$ to move from the atomic scale to the macroscopic scale.
...
${\(  {p~=~D_C M_F \over D_C }\)}^W {\({299792458 \text{Meters}\over D_C }\)}^X {\({1 \text{Second}\over D_C }\)}^Y {\({q~=~D_C q_e\over D_C }\)}^Z$
$${{h ~ \alpha} \over c~ q_e^2}{ 2 c^2 \over 4\pi }={ \mu _{0}~c^2 \over 4\pi }={1\over 4 \pi \epsilon _0} = k_e = ~\alpha {\({c{s\over m}}\)}^{3} p^{1}m^{3}s^{-2}q^{-2}$$
\begin{multicols}{2}[\setlength{\columnseprule}{0pt}]\noindent\begin{align*}
...
N_A = D_C\\
\alpha={{\mu _{0} }\over 2}{c ~ q_e^2 \over 2~\pi ~\hbar}\\
%constant = ~1 ~&p^{}m^{}s^{}q^{}\\
%constant = ~1 ~&p^{}m^{}s^{}q^{}\\
...
%k_e = ~\alpha {\({c{s\over m}}\)}^{3}& p^{1}m^{3}s^{-2}q^{-2}\\
%$${{h ~ \alpha} \over c~ q_e^2}{ 2 c^2 \over 4\pi }={ \mu _{0}~c^2 \over 4\pi }={1\over 4 \pi \epsilon _0}=k_e= ~\alpha {\({c{s\over m}}\)}^{3}& p^{1}m^{3}s^{-2}q^{-2}$$
%= ~\alpha {\({c{s\over m}}\)}^{3}& p^{1}m^{3}s^{-2}q^{-2}
...
%$\hspace{2.5em} k_e ={10^{−7} ~c^2 }{T^2 \over L^2}{M~L^3 \over T^2~Q^2}$
% \begin{figure}[h]
%   \centering
...
This section is paraphrased almost verbatim from \citep{Horst1}. % and \citep{https://doi.org/10.1002/andp.201800308}
